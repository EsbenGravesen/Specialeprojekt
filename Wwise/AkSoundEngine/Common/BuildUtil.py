...
SupportedPlatforms = \
{
	'Windows': ['Windows', 'Android', 'WSA', 'PS3', 'PS4', 'XBox360', 'XBoxOne', 'Vita', 'Linux', 'WiiU'],
	'Darwin': ['Mac', 'iOS', 'Android', 'tvOS'],
	'Linux': ['Linux']
}
...
# Platforms that share banks with other platforms.
RedundantBankPlatforms = ['WSA']
...
SupportedArches = \
{
	...
	'WSA': ['WSA_UWP_Win32', 'WSA_UWP_ARM', 'WSA_UWP_x64'],
	'Windows': ['Win32', 'x64'],
	'Vita': ['SW', 'HW'],
	'Linux': ['x86', 'x86_64']
}
...
PlatformSwitches = \
{
	'Windows': '#if (UNITY_STANDALONE_WIN && !UNITY_EDITOR) || UNITY_EDITOR_WIN',
	'Mac': '#if (UNITY_STANDALONE_OSX && !UNITY_EDITOR) || UNITY_EDITOR_OSX',
	...
	'tvOS': '#if UNITY_TVOS && ! UNITY_EDITOR',
	'WSA': '#if UNITY_WSA && ! UNITY_EDITOR',
	'Vita': '#if UNITY_PSP2 && !UNITY_EDITOR',
	'Linux': '#if UNITY_STANDALONE_LINUX && ! UNITY_EDITOR',
	'WiiU': '#if UNITY_WIIU && !UNITY_EDITOR',
	'Common': '#if ! (UNITY_DASHBOARD_WIDGET || UNITY_WEBPLAYER || UNITY_WII || UNITY_NACL || UNITY_FLASH || UNITY_BLACKBERRY) // Disable under unsupported platforms.'
}
...
[
	...
	'AkAudioSessionCategory.{}'.format(ApiExt), # iOS only
	'AkAudioSessionCategoryOptions.{}'.format(ApiExt), # iOS only
	'AkAudioSessionMode.{}'.format(ApiExt), # iOS only
	'AkAudioSessionProperties.{}'.format(ApiExt), # iOS only
	'AkAudioSessionSetActiveOptions.{}'.format(ApiExt), # iOS only
	'AkOutputSelector.cs' #WiiU specific
]
...
class PathManager(object):
	def __init__(self, PlatformName, Arch=None, wwiseSdkRootDir=None):
		...
		self.Paths['Deploy'] = self.NormJoin(self.Paths['Root'], 'Integration', 'Assets', 'Wwise', 'Deployment')
		...
		self.Paths['Deploy_Components'] = self.NormJoin(self.Paths['Deploy'], 'Components')
...
class PluginBuildProduct(object):
	def __init__(self, pathMan, Config):
		...
		self.Config = Config
		self.Binaries = [] # Files that go to the Integration/Assets/Wwise/Deployment/Plugins/Platform/Arch/Configuration folder.
		self.CommonBinaries = [] # Files that go to the Integration/Assets/Wwise/Deployment/Plugins/Platform folder.
		self.EditorScripts = []
		self.BuildDir = None
...
class tvOSProduct(XcodeProduct):
	def __init__(self, pathMan, Config, BuildDir):
		XcodeProduct.__init__(self, pathMan, Config, BuildDir)
...
class NdkProduct(PluginBuildProduct):
	def __init__(self, pathMan, Config, Arch):
		...
		self.Binaries = ['lib{}.so'.format(self.pathMan.ProductName)]		
	...
	def PreProcess(self):
		if (not self.BuildDir is None) and exists(self.BuildDir):
			# NDK uses separate buildDirs for .so.  To ease things up, we copy to .so folder to use just one buildDir as we do other platforms.
