...
class ApiRepairman(object):
	...
	def MakePlatformApiSwitchable(self):
		...
		self.__BatchMakeApiBypassable([self.pathMan.Paths['Deploy_Components']], 'Common')
	...
	def __MakeApiBypassable(self, apiPath, platformKey):
		...
		hasSwitchHead = not switchHeadRegEx.search(allLines[0]) is None
		hasSwitchTail = not switchTailRegEx.search(allLines[len(allLines)-1]) is None
		if hasSwitchHead:
			allLines[0] = switchHead
		else:
			allLines = [switchHead] + allLines
		if hasSwitchTail:
			allLines[len(allLines)-1] = switchTail
		else:
			allLines.append(switchTail)
...
def main(pathMan=None):
	...
	isMultiArchPlatform = pathMan.PlatformName in BuildUtil.SupportedArches.keys() and pathMan.PlatformName in BuildUtil.AkMultiArchPlatforms[platform.system()] and pathMan.PlatformName in BuildUtil.MultiArchSwitches
